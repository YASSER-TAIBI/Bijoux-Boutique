...
import { Review, ReviewResponse, CreateReviewRequest, ReviewStats, RatingStats } from '../models/review.interface';
...
export class ReviewService {
  ...
  updateReviewVisibility(reviewId: string, isVisible: boolean): Observable<any> {
    return this.http.patch<any>(`${this.apiUrl}/admin/${reviewId}/visibility`, 
      { isVisible },
      { headers: this.getAuthHeaders() }
    );
  }
  ...
  deleteReview(reviewId: string): Observable<any> {
    return this.http.delete<any>(`${this.apiUrl}/admin/${reviewId}`, {
      headers: this.getAuthHeaders()
    });
  }
